...
| 핵심 역량 | 상세 목표 |
| :--- | :--- |
| **함수(Tool) 만들기** | `@tool` 표시, 타입 힌트, 함수 설명(docstring)을 꼼꼼히 적어 LLM이 "이 함수를 언제·어떻게 부르면 되는지" 스스로 판단할 수 있게 만들기 |
| **외부 데이터 연결** | 웹페이지에서 환율 가져오기(`requests` + `BeautifulSoup4`), 도시별 시간대 계산(`zoneinfo`), SQLite로 직원 DB 흉내 내기 |
| **만들어진 툴킷 활용** | `SQLDatabaseToolkit`이 자동으로 만들어 주는 4개 함수(테이블 목록 보기 / 컬럼 보기 / 쿼리 확인 / 쿼리 실행)를 그대로 사용해 자연어 질문을 SQL로 자동 변환 |
| **안전한 챗봇 만들기** | "삭제·수정 같은 위험한 명령은 실행하지 않는다"는 규칙을 시스템 프롬프트에 적어 두어, 사용자가 위험한 요청을 해도 챗봇이 거절하도록 설계 |
...
| :---: | :--- | :--- | :---: |
| 시간 조회 | `get_global_office_times` | 해외 7개 도시의 현재 시각을 한 번에 계산해서 반환 | `zoneinfo` |
| 웹페이지 긁어오기 | `get_usd_krw_exchange_rate` | 네이버 증권 페이지의 환율 위치를 찾아 텍스트로 추출 | `requests` · `BeautifulSoup4` |
| SQL 툴킷 | `sql_db_list_tables` | DB 안에 어떤 테이블이 있는지 목록 반환 | `SQLDatabaseToolkit` |
| SQL 툴킷 | `sql_db_schema` | 테이블의 컬럼 구조와 샘플 3줄을 LLM에 보여줌 | `SQLDatabaseToolkit` |
| SQL 툴킷 | `sql_db_query_checker` | 실행 전 SQL 문법이 맞는지 한 번 더 검사 | `SQLDatabaseToolkit` |
| SQL 툴킷 | `sql_db_query` | 검사를 통과한 쿼리를 실제로 실행 | `SQLDatabaseToolkit` |

### 3. 핵심 설계 포인트

| 설계 포인트 | 적용 방법 | 효과 |
| :--- | :--- | :--- |
| **함수 설명이 곧 LLM 사용 설명서** | `@tool` 함수의 docstring에 **언제·어떻게 부르는지**를 풀어서 적음 | LLM은 함수 안의 코드를 읽지 않고, 설명문과 타입 힌트만 보고 호출을 결정함 |
| **DB 종류를 자동으로 알려 주기** | 시스템 프롬프트에 `db.dialect`(예: `sqlite`) 값을 직접 끼워 넣음 | 나중에 같은 코드로 MySQL이나 PostgreSQL을 써도 LLM이 알아서 올바른 SQL을 만들어 줌 |
| **결과 개수 제한** | "결과는 최대 5건까지만 가져온다"고 시스템 프롬프트에 명시 | 한 번에 수천 줄이 쏟아져 들어와 답변 비용이 크게 늘어나는 상황을 미리 막음 |
| **읽기 전용 규칙** | "삭제·수정·생성 같은 변경 명령은 실행하지 않고 정중히 거절한다"는 규칙을 시스템 프롬프트에 못 박음 | 사용자가 위험한 요청을 해도 LLM이 SQL 함수를 부르기 전에 알아서 거절 |
| **외부 사이트 오류 대응** | 웹페이지 요청 함수에 5초 타임아웃과 `try/except`를 적용 | 네이버 증권 같은 외부 사이트가 점검·장애일 때도 챗봇 전체가 멈추지 않고 친절한 메시지로 답변 |
...
| # | 발견한 사실 | 관찰 내용 | 그래서 어떻게 적용해야 하는가 |
| :---: | :--- | :--- | :--- |
| 1 | **함수 설명이 곧 LLM 사용 설명서** | 함수 설명(docstring)을 비워 둔 채 등록했을 때, "지금 몇 시야?" 같은 시간 질문에도 LLM이 함수를 부르지 않고 "실시간 시간을 알 수 없다"는 일반 답변만 함. 사용 안내 한 줄을 추가하자 즉시 정확히 호출함 | LLM은 함수 안의 코드를 읽지 않고 **함수 이름·타입 힌트·설명문**만 본다는 점을 기억하기. 이 세 가지를 합쳐 "LLM이 읽을 사용 설명서"라고 생각하고 꼼꼼히 적어야 함 |
| 2 | **SQL 툴킷의 4단계 사고 흐름** | `sql_db_query` 함수 하나만 있어도 동작은 하지만, `테이블 목록 → 컬럼 보기 → 쿼리 검사 → 쿼리 실행` 4단계를 거치게 하면 LLM이 **DB 구조를 전혀 모르는 상태에서도** 안전하게 정답에 도달함 | 사람 개발자가 처음 보는 DB를 만났을 때 거치는 절차(테이블 살피기 → 컬럼 보기 → 쿼리 짜기 → 실행)와 거의 똑같음. 함수를 단순히 모아 두는 게 아니라 "절차" 자체를 함수로 풀어 두는 발상이 인상적 |
| 3 | **시스템 프롬프트가 첫 번째 안전장치** | "삭제·수정 명령은 실행하지 않는다"는 한 줄을 시스템 프롬프트에 넣었을 뿐인데, "박지수 대리를 삭제해줘" 요청이 들어오자 LLM이 SQL 함수를 부르지 않고 정중히 거절함 | 시스템 프롬프트는 사실상 챗봇의 행동 규칙. 다만 이건 **첫 번째 방어선**일 뿐이고, 실제 서비스라면 DB 사용자 계정 자체를 "읽기만 가능"으로 만드는 게 정석 |
| 4 | **외부 사이트 연결은 평범한 코딩 기본기** | 환율 함수는 네이버 증권 페이지 구조가 바뀌면 즉시 깨짐. 5초 타임아웃과 `try/except`로 실패 메시지를 친절히 돌려주도록 하자 챗봇 답변이 훨씬 안정적이 됨 | LLM이 아무리 똑똑해도 함수가 깨지면 챗봇 전체가 멈춤. **타임아웃·예외 처리·명확한 실패 메시지** 같은 평범한 코딩 기본기가 AI 시스템에서도 그대로 중요함 |
