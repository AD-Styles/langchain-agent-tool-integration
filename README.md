...
| :--- | :--- |
| **함수 설명이 곧 LLM 사용 설명서** | `@tool` 함수의 docstring에 "언제·어떻게 부르는지"를 풀어서 적어 둠. LLM은 함수 안의 코드를 <br>읽지 않고 설명문과 타입 힌트만 보고 호출을 결정함 |
| **DB 종류를 자동으로 알려 주기** | 시스템 프롬프트에 `db.dialect`(예: `sqlite`) 값을 직접 끼워 넣음. 나중에 같은 코드로 MySQL <br>이나 PostgreSQL을 써도 LLM이 알아서 올바른 SQL을 만들어 줌 |
