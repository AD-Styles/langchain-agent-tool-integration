...
| 설계 포인트 &emsp;&emsp;&emsp; | 적용 방법과 효과 |
| :--- | :--- |
| **함수 설명이 곧 LLM <br>사용 설명서** | `@tool` 함수의 docstring에 "언제·어떻게 부르는지"를 풀어서 적어 둠. LLM은 함수 안의 코드를 <br>읽지 않고 설명문과 타입 힌트만 보고 호출을 결정함 |
| **DB 종류를 자동으로 <br>알려 주기** | 시스템 프롬프트에 `db.dialect`(예: `sqlite`) 값을 직접 끼워 넣음. 나중에 같은 코드로 MySQL <br>이나 PostgreSQL을 써도 LLM이 알아서 올바른 SQL을 만들어 줌 |
| **결과 개수 제한** | "결과는 최대 5건까지만 가져온다"를 시스템 프롬프트에 명시. 한 번에 수천 줄이 쏟아져 답변 비용이 크게 늘어나는 상황을 미리 막음 |
| **읽기 전용 규칙** | "삭제·수정·생성 같은 변경 명령은 실행하지 않고 정중히 거절한다"는 규칙을 시스템 프롬프트에 <br>못 박음. 사용자가 위험한 요청을 해도 LLM이 SQL 함수를 부르기 전에 알아서 거절 |
| **외부 사이트 오류 <br>대응** | 웹페이지 요청 함수에 5초 타임아웃과 `try/except`를 적용. 외부 사이트가 점검·장애일 때도 챗봇 전체가 멈추지 않고 친절한 메시지로 답변 |
