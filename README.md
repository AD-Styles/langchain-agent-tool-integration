...
| :---: | :--- | :--- | :--- |
| 1 | **함수 설명이 곧<br/>LLM 사용 설명서** | 함수 설명(docstring)을 비워 둔 채 등록했을 때,<br/>"지금 몇 시야?" 같은 시간 질문에도 LLM이<br/>함수를 부르지 않고 "실시간 시간을 알 수 없다"는<br/>일반 답변만 함. 사용 안내 한 줄을 추가하자<br/>즉시 정확히 호출함 | LLM은 함수 안의 코드를 읽지 않고<br/>**함수 이름·타입 힌트·설명문**만 본다는 점을 기억하기.<br/>이 세 가지를 합쳐 "LLM이 읽을 사용 설명서"라고<br/>생각하고 꼼꼼히 적어야 함 |
| 2 | **SQL 툴킷의<br/>4단계 사고 흐름** | `sql_db_query` 함수 하나만 있어도 동작은 하지만,<br/>`테이블 목록 → 컬럼 보기 → 쿼리 검사 → 쿼리 실행`<br/>4단계를 거치게 하면 LLM이<br/>**DB 구조를 전혀 모르는 상태에서도**<br/>안전하게 정답에 도달함 | 사람 개발자가 처음 보는 DB를 만났을 때 거치는 절차<br/>(테이블 살피기 → 컬럼 보기 → 쿼리 짜기 → 실행)와<br/>거의 똑같음. 함수를 단순히 모아 두는 게 아니라<br/>"절차" 자체를 함수로 풀어 두는 발상이 인상적 |
| 3 | **시스템 프롬프트가<br/>첫 번째 안전장치** | "삭제·수정 명령은 실행하지 않는다"는 한 줄을<br/>시스템 프롬프트에 넣었을 뿐인데,<br/>"박지수 대리를 삭제해줘" 요청이 들어오자<br/>LLM이 SQL 함수를 부르지 않고 정중히 거절함 | 시스템 프롬프트는 사실상 챗봇의 행동 규칙.<br/>다만 이건 **첫 번째 방어선**일 뿐이고,<br/>실제 서비스라면 DB 사용자 계정 자체를<br/>"읽기만 가능"으로 만드는 게 정석 |
| 4 | **외부 사이트 연결은<br/>평범한 코딩 기본기** | 환율 함수는 네이버 증권 페이지 구조가 바뀌면<br/>즉시 깨짐. 5초 타임아웃과 `try/except`로<br/>실패 메시지를 친절히 돌려주도록 하자<br/>챗봇 답변이 훨씬 안정적이 됨 | LLM이 아무리 똑똑해도 함수가 깨지면<br/>챗봇 전체가 멈춤.<br/>**타임아웃·예외 처리·명확한 실패 메시지** 같은<br/>평범한 코딩 기본기가 AI 시스템에서도 그대로 중요함 |
