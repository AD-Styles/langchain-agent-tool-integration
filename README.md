...
| 핵심 역량  | 상세 목표 |
| :--- | :--- |
| **함수(Tool) <br>만들기** | `@tool` 표시, 타입 힌트, 함수 설명(docstring)을 꼼꼼히 적어 LLM이 "이 함수를 언제·어떻게 부르면 되는지" 스스로 판단할 수 있게 만들기 |
...
| 분류 &emsp;&emsp;&emsp; | 함수 이름 | 핵심 동작 | 사용 라이브러리 |
| :---: | :--- | :--- | :---: |
| 시간 조회 | `get_global_office_times` | 해외 7개 도시의 현재 시각을 한 번에 계산해서 반환 | `zoneinfo` |
| 웹페이지<br/>긁어오기 | `get_usd_krw_exchange_rate` | 네이버 증권 페이지의 환율 위치를 찾아 텍스트로 추출 | `requests`<br/>`BeautifulSoup4` |
...
| :--- | :--- |
| **함수 설명이 곧 LLM <br/>사용 설명서** | `@tool` 함수의 docstring에 "언제·어떻게 부르는지"를 풀어서 적어 둠. LLM은 함수 안의 코드를 <br>읽지 않고 설명문과 타입 힌트만 보고 호출을 결정함 |
| **DB 종류를 자동으로 <br/>알려 주기** | 시스템 프롬프트에 `db.dialect`(예: `sqlite`) 값을 직접 끼워 넣음. 나중에 같은 코드로 MySQL 이나 PostgreSQL을 써도 LLM이 알아서 올바른 SQL을 만들어 줌 |
| **결과 개수 제한** | "결과는 최대 5건까지만 가져온다"를 시스템 프롬프트에 명시. 한 번에 수천 줄이 쏟아져 답변 <br>비용이 크게 늘어나는 상황을 미리 막음 |
| **읽기 전용 규칙** | "삭제·수정·생성 같은 변경 명령은 실행하지 않고 정중히 거절한다"는 규칙을 시스템 프롬프트에 <br>못 박음. 사용자가 위험한 요청을 해도 LLM이 SQL 함수를 부르기 전에 알아서 거절 |
| **외부 사이트 오류 대응** | 웹페이지 요청 함수에 5초 타임아웃과 `try/except`를 적용. 외부 사이트가 점검·장애일 때도 <br>챗봇 전체가 멈추지 않고 친절한 메시지로 답변 |

### 4. 실행 방법

| 명령어 &emsp;&emsp;&emsp;&emsp;&emsp;&emsp; | 동작 &emsp;&emsp;&emsp;&emsp; | LLM 호출 |
| :--- | :--- | :---: |
...
| :--- | :--- |
| **함수 설명이 곧 LLM 사용 설명서** | docstring을 비워 둔 채 등록하니 "지금 몇 시야?" 질문에도 LLM이 함수를 부르지 않았음. 사용 안내 한 줄을 추가하자 즉시 정확히 호출함. → LLM은 함수 코드를 읽지 않고 **이름·타입 힌트·설명문**만 본다. <br>이 세 가지를 "LLM이 읽을 사용 설명서"로 보고 꼼꼼히 적어야 함 |
| **SQL 툴킷의 4단계 사고 흐름** | `테이블 목록 → 컬럼 보기 → 쿼리 검사 → 쿼리 실행` 4단계를 거치게 하면 LLM이 DB 구조를 전혀 모르는 상태에서도 안전하게 정답에 도달함. → 사람 개발자가 처음 보는 DB를 다루는 절차와 거의 동일. <br>함수를 모아 두는 게 아니라 **"절차" 자체를 함수로 풀어 둔 설계** 가 인상적 |
