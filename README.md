...
| 분류 &emsp; | 함수 이름 &emsp;&emsp; | 핵심 동작 &emsp;&emsp;&emsp;&emsp; | 사용 라이브러리 |
| :---: | :--- | :--- | :---: |
...
| :--- | :--- |
| **함수 설명이 곧 LLM 사용 설명서** | `@tool` 함수의 docstring에 "언제·어떻게 부르는지"를 풀어서 적어 둠. LLM은 함수 안의 코드를 읽지 않고 설명문과 타입 힌트만 보고 호출을 결정함 |
...
| **외부 사이트 오류 대응** | 웹페이지 요청 함수에 5초 타임아웃과 `try/except`를 적용. 외부 사이트가 점검·장애일 때도 챗봇 전체가 멈추지 않고 친절한 메시지로 답변 |
